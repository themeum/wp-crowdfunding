...
function woocommerce_twocheckout(){
	...
	class WC_Gateway_Twocheckout extends WC_Payment_Gateway{
		...
		public function admin_options() {
			?>
            <h3><?php _e( '2Checkout', 'wp-crowdfunding' ); ?></h3>
            <p><?php _e( '2Checkout - Credit Card/Paypal', 'wp-crowdfunding' ); ?></p>
			...
			<?php else : ?>
                <div class="inline error"><p><strong><?php _e( 'Gateway Disabled', 'wp-crowdfunding' ); ?></strong>: <?php _e( '2Checkout does not support your store currency.', 'wp-crowdfunding' ); ?></p></div>
				<?php
			endif;
		}
		...
		function init_form_fields() {
			...
			$this->form_fields = array(
				'enabled' => array(
					...
					'type'          => 'checkbox',
					'label'         => __( 'Enable 2Checkout', 'wp-crowdfunding' ),
					'default'       => 'yes'
				),
				'title' => array(
					...
					'default'       => __( 'Credit Card/PayPal', 'wp-crowdfunding' ),
					'desc_tip'      => true,
				),
				'description' => array(
					...
					'default'       => __( 'Pay with Credit Card/PayPal', 'wp-crowdfunding' )
				),
				'icons' => array(
					'title'         => __( 'Icon URL', 'wp-crowdfunding' ),
					'type'          => 'text',
					'description'   => __( 'This controls the Icon URL which the user sees during checkout.', 'wp-crowdfunding' ),
					'default'       => '',
					'desc_tip'      => true,
				),
				'seller_id' => array(
					'title'         => __( 'Seller ID', 'wp-crowdfunding' ),
					'type'          => 'text',
					'description'   => __( 'Please enter your 2Checkout account number; this is needed in order to take payment.', 'wp-crowdfunding' ),
					'default'       => '',
					'desc_tip'      => true,
					'placeholder'   => ''
				),
				'publishable_key' => array(
					'title'         => __( 'Publishable Key', 'wp-crowdfunding' ),
					'type'          => 'text',
					'description'   => __( 'Please enter your 2Checkout Publishable Key; this is needed in order to take payment.', 'wp-crowdfunding' ),
					'default'       => '',
					'desc_tip'      => true,
					'placeholder'   => ''
				),
				'private_key' => array(
					'title'         => __( 'Private Key', 'wp-crowdfunding' ),
					'type'          => 'text',
					'description'   => __( 'Please enter your 2Checkout Private Key; this is needed in order to take payment.', 'wp-crowdfunding' ),
					'default'       => '',
					'desc_tip'      => true,
					'placeholder'   => ''
				),
				'sandbox' => array(
					'title'         => __( 'Sandbox/Production', 'wp-crowdfunding' ),
					'type'          => 'checkbox',
					'label'         => __( 'Use 2Checkout Sandbox', 'wp-crowdfunding' ),
					'default'       => 'no'
				),
				'debug' => array(
					'title'         => __( 'Debug Log', 'wp-crowdfunding' ),
					'type'          => 'checkbox',
					'label'         => __( 'Enable logging', 'wp-crowdfunding' ),
					'default'       => 'no',
					'description'   => sprintf( __( 'Log 2Checkout events', 'wp-crowdfunding' ), wc_get_log_file_path( 'twocheckout' ) )
				),
		...
		function payment_fields() {
            ...
            <fieldset>
                ...
                <p class="form-row form-row-first">
                    <label for="ccNo"><?php echo __( 'Credit Card number', 'wp-crowdfunding' ) ?> <span class="required">*</span></label>
                ...
                <p class="form-row form-row-first">
                    <label for="cc-expire-month"><?php echo __( 'Expiration date', 'wp-crowdfunding') ?> <span class="required">*</span></label>
                    <select id="expMonth" class="woocommerce-select woocommerce-cc-month">
                        <option value=""><?php _e( 'Month', 'wp-crowdfunding' ) ?></option><?php
						$months = array();
						for ( $i = 1; $i <= 12; $i ++ ) {
                    ...
                    <select id="expYear" class="woocommerce-select woocommerce-cc-year">
                        <option value=""><?php _e( 'Year', 'wp-crowdfunding' ) ?></option>
						<?php
						$years = array();
                ...
                <p class="form-row">
                    <label for="cvv"><?php _e( 'Card security code', 'wp-crowdfunding' ) ?> <span class="required">*</span></label>
                    <input type="text" class="input-text" id="cvv" autocomplete="off" maxlength="4" style="width:55px" />
                    <span class="help"><?php _e( '3 or 4 digits usually found on the signature strip.', 'wp-crowdfunding' ) ?></span>
                </p>
		...
		function receipt_page( $order ) {
			echo '<p>'.__( 'Thank you for your order, please click the button below to pay with PayPal.', 'wp-crowdfunding' ).'</p>';
