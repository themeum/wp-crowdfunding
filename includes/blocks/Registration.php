...
class Registration {
			...
			<div class="wpneo-user-registration-wrap">
				...
				<?php echo wp_kses_post( wp_nonce_field( 'wpcf_form_action', 'wpcf_form_action_field', true, false ) ); ?>
					...
					foreach ( $regisration_meta as $item ) {
						?>
						<div class="wpneo-single <?php echo esc_attr( ( isset( $item['warpclass'] ) ? $item['warpclass'] : '' ) ); ?>">
							<div class="wpneo-name"><?php echo esc_html( ( isset( $item['label'] ) ? $item['label'] : '' ) ); ?></div>
							<div class="wpneo-fields">
								<?php
								switch ( $item['type'] ) {
									case 'text':
										echo '<input type="text" id="' . esc_attr( $item['id'] ) . '" autocomplete="' . esc_attr( $item['autocomplete'] ) . '" class="' . esc_attr( $item['class'] ) . '" name="' . esc_attr( $item['id'] ) . '" placeholder="' . esc_attr( $item['placeholder'] ) . '">';
										break;
									case 'password':
										echo '<input type="password" id="' . esc_attr( $item['id'] ) . '" autocomplete="' . esc_attr( $item['autocomplete'] ) . '" class="' . esc_attr( $item['class'] ) . '" name="' . esc_attr( $item['id'] ) . '" placeholder="' . esc_attr( $item['placeholder'] ) . '">';
										break;
									case 'textarea':
										echo '<textarea id="' . esc_attr( $item['id'] ) . '" autocomplete="' . esc_attr( $item['autocomplete'] ) . '" class="' . esc_attr( $item['class'] ) . '" name="' . esc_attr( $item['id'] ) . '" ></textarea>';
										break;
									case 'submit':
										echo '<input type="submit" id="' . esc_attr( $item['id'] ) . '"  class="' . esc_attr( $item['class'] ) . '" name="' . esc_attr( $item['id'] ) . '" />';
										break;
									case 'shortcode':
										echo do_shortcode( esc_attr( $item['shortcode'] ) );
										break;
								}
								?>
							</div>
					...
					<div class="wpneo-single wpneo-register">
						<a href="<?php echo esc_url( get_home_url() ); ?>" class="wpneo-cancel-campaign" style="background: <?php echo esc_attr( $cancelbtnbgColorpalette ); ?>; color: <?php echo esc_attr( $cancelbtncolor ); ?>; font-size: <?php echo esc_attr( $cancelfontSize ); ?>px; font-weight: <?php echo esc_attr( $cancelfontWeight ); ?>"><?php esc_html_e( 'Cancel', 'wp-crowdfunding' ); ?></a>
						<input type="hidden" name="action" value="wpcf_registration" />
						<input type="hidden" name="current_page" value="<?php echo esc_url( get_the_permalink() ); ?>" />
						<input type="submit" class="wpneo-submit-campaign" id="user-registration-btn" value="<?php esc_html_e( 'Sign UP', 'wp-crowdfunding' ); ?>" name="submits" style="background: <?php echo esc_attr( $bgColor ); ?>; color: <?php echo esc_attr( $titleColor ); ?>; font-size: <?php echo esc_attr( $fontSize ); ?>px; font-weight: <?php echo esc_attr( $fontWeight ); ?>"/>
					</div>
					...
					<style>
						.wpneo-fields input[type="number"], .wpneo-fields input[type="text"], .wpneo-fields input[type="email"], .wpneo-fields input[type="password"] {
							font-size: <?php echo esc_attr( $inputfontSize ); ?>px;
							color: <?php echo esc_attr( $inputTextColor ); ?>;
							border: 1px solid <?php echo esc_attr( $borderColor ); ?>px;
						}
						.wpneo-user-registration-wrap .wpneo-name {
							font-size: <?php echo esc_attr( $labelfontSize ); ?>px;
	...
	public function registration_save_action() {
		if ( ! isset( $_POST['wpcf_form_action_field'] ) || ! wp_verify_nonce( sanitize_text_field( $_POST['wpcf_form_action_field'] ), 'wpcf_form_action' ) ) {
			die(
				json_encode(
					array(
						'success' => 0,
						'message' => __( 'Sorry, your status did not verify.', 'wp-crowdfunding' ),
					)
				)
			);
			exit;
			...
			if ( ! is_wp_error( $user_id ) ) {
				...
				die(
					json_encode(
						array(
							'success'  => 1,
							'message'  => __( 'Registration complete.', 'wp-crowdfunding' ),
							'redirect' => $redirect,
						)
					)
				);
