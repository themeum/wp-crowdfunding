	...
	while ( $the_query->have_posts() ) :
		...
		<div class="wpneo-listings-dashboard wpneo-shadow wpneo-padding15 wpneo-clearfix">
			...
			<div class="wpneo-listing-img">
				<a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php echo esc_html( get_the_title() ); ?>"> <?php echo woocommerce_get_product_thumbnail(); ?></a>
				<div class="overlay">
					<div>
						<div>
							<a class="wp-crowd-btn wp-crowd-btn-primary" href="<?php echo esc_url( get_permalink() ); ?>"><?php esc_html_e( 'View', 'wp-crowdfunding' ); ?></a>
						</div>
					</div>
				</div>
			</div>
			...
			<div class="wpneo-listing-content clearfix">
				...
				<div class="wpneo-admin-title float-left">
					...
					<p class="wpneo-author"><?php esc_html_e( 'by', 'wp-crowdfunding' ); ?> 
						<a href="<?php echo esc_attr( wpcf_function()->get_author_url( get_the_author_meta( 'user_login' ) ) ); ?>"><?php echo wpcf_function()->get_author_name(); ?></a>
					</p>
					...
					<?php if ( $location ) { ?>
						<div class="wpneo-location">
							...
							<div class="wpneo-meta-desc"><?php echo esc_html_e( $location ); ?></div>
						</div>
					<?php } ?>
				</div>
				<div class="wpneo-admin-location float-right">
					...
					if ( $page_id != '' ) {
						...
						$permalink_update = add_query_arg(
							array(
								'page_type' => 'update',
								'postid'    => get_the_ID(),
							),
					...
					$operation_btn .= '</div>';
					echo wp_kses_post( $operation_btn );
					?>
				</div>
				...
				<div class="wpneo-percent-rund-wrap">
					...
					<div class="crowdfound-pie-chart" data-size="60" data-percent="<?php echo esc_attr( $raised_percent ); ?>">
						<div class="sppb-chart-percent"><span><?php echo esc_html( $raised_percent ); ?></span></div>
					</div>
					...
					if ( $end_method != 'never_end' ) {
						...
						<div class="crowdfound-time-remaining">
							<?php if ( wpcf_function()->is_campaign_started() ) { ?>
								<div class="wpneo-meta-desc"><?php echo esc_html( wpcf_function()->get_date_remaining() ); ?></div>
								<div class="wpneo-meta-name"><?php esc_html_e( 'Days to go', 'wp-crowdfunding' ); ?></div>
							<?php } else { ?>
								<div class="wpneo-meta-desc"><?php echo esc_html( wpcf_function()->days_until_launch() ); ?></div>
								<div class="wpneo-meta-name"><?php esc_html_e( 'Days Until Launch', 'wp-crowdfunding' ); ?></div>
							<?php } ?>
						</div>
					<?php } ?>
