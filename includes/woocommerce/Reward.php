		...
		<div id='reward_options' class='panel woocommerce_options_panel'>
			...
			foreach ( $woocommerce_meta_field as $value ) {
				switch ( $value['field_type'] ) {
					...
					case 'image':
						...
						echo '<label for="wpneo_rewards_image_field">' . esc_html( $value['label'] ) . '</label>';
						echo '<input type="hidden" class="wpneo_rewards_image_field" name="' . esc_attr( $value['id'] ) . '" value="" placeholder="' . esc_attr( $value['label'] ) . '"/>';
						echo '<span class="wpneo-image-container"></span>';
						echo '<button class="wpneo-image-upload-btn shorter">' . esc_html__( 'Upload', 'wp-crowdfunding' ) . '</button>';
						echo '</p>';
						break;
			...
			*/
			...
			if ( wpcf_function()->is_free() ) {
				...
				<input name="save" type="button" class="button button-primary tagadd" id="addreward" value="<?php esc_html_e( '+ Add Reward', 'wp-crowdfunding' ); ?>">
			<?php } ?>
		</div>
			...
			<tr>
				<td>
					...
					if ( isset( $reward_data['wpneo_rewards_description'] ) ) {
						echo '<div>' . wp_kses_post( wpautop( $reward_data['wpneo_rewards_description'] ) ) . '</div>';
						echo esc_html__( 'Amount', 'wp-crowdfunding' ) . ' : <strong>' . wp_kses_post( wc_price( $reward_data['wpneo_rewards_pladge_amount'] ) ) . '</strong>, ';
						echo esc_html__( 'Delivery', 'wp-crowdfunding' ) . ' : ' . esc_html( $reward_data['wpneo_rewards_endmonth'] ) . ', ' . esc_html( $reward_data['wpneo_rewards_endyear'] );
					}
					?>
				</td>
				<td></td>
