...
class Submit_Form {
	...
	/**
	 * Save frontend campaign submit form data.
	 *
	 * @return void
	 *
	 * Save frontend campaign submit form data.
		...
		if ( ! isset( $_POST['wpcf_form_action_field'] ) || ! wp_verify_nonce( $_POST['wpcf_form_action_field'], 'wpcf_form_action' ) ) {
			die(
				json_encode(
					array(
						'success' => 0,
						'message' => __( 'Sorry, your data did not verify.', 'wp-crowdfunding' ),
					)
				)
			);
			exit;
		...
		if ( ! current_user_can( 'campaign_form_submit' ) ) {
			die(
				json_encode(
					array(
						'success' => 0,
						'message' => __( 'You are not allowed to submit campaigns.', 'wp-crowdfunding' ),
					)
				)
			);
			exit;
		...
		if ( empty( $_POST['wpneo-form-title'] ) ) {
			die(
				json_encode(
					array(
						'success' => 0,
						'message' => __( 'Title required', 'wp-crowdfunding' ),
					)
				)
			);
		}
		if ( empty( $_POST['wpneo-form-description'] ) && $show_description == 'true' ) {
			die(
				json_encode(
					array(
						'success' => 0,
						'message' => __( 'Description required', 'wp-crowdfunding' ),
					)
				)
			);
		}
		if ( empty( $_POST['wpneo-form-short-description'] ) && $show_short_description == 'true' ) {
			die(
				json_encode(
					array(
						'success' => 0,
						'message' => __( 'Short Description required', 'wp-crowdfunding' ),
					)
				)
			);
		}
		if ( empty( $_POST['wpneo-form-funding-goal'] ) && $show_funding_goal == 'true' ) {
			die(
				json_encode(
					array(
						'success' => 0,
						'message' => __( 'Funding goal required', 'wp-crowdfunding' ),
					)
				)
			);
		}
		if ( empty( $_POST['wpneo_terms_agree'] ) && $show_terms_and_conditions == 'true' ) {
			die(
				json_encode(
					array(
						'success' => 0,
						'message' => __( 'Please check terms condition', 'wp-crowdfunding' ),
					)
				)
			);
		}
