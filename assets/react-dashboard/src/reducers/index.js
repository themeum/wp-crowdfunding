...
const reducer = (state = defaultState, action) => {
    switch( action.type ) {
        ...
        case FETCH_CAMPAIGNS_REPORT_PENDING: {
            const campaignsReport = { ...state.campaignsReport };
        ...
        case FETCH_MY_CAMPAIGNS_PENDING: {
            const myCampaign = { ...state.myCampaign };
        ...
        case SAVE_CAMPAIGN_UPDATES_COMPLETE: {
            const myCampaign = { ...state.myCampaign };
            const campaignData = [ ...myCampaign.data ];
            const index = campaignData.findIndex(item => item.id == action.payload.id);
        ...
        case DELETE_CAMPAIGN_COMPLETE: {
            const myCampaign = { ...state.myCampaign };
            const campaignData = [ ...myCampaign.data ];
            const index = campaignData.findIndex(item => item.id == action.payload);
        ...
        case FETCH_INVESTED_CAMPAIGNS_PENDING: {
            const investedCampaign = { ...state.investedCampaign };
        ...
        case FETCH_BOOKMARK_CAMPAIGNS_PENDING: {
            const bookmarkCampaign = { ...state.bookmarkCampaign };
        ...
        case DELETE_BOOKMARK_CAMPAIGN_COMPLETE: {
            const bookmarkCampaign = { ...state.bookmarkCampaign };
            const campaignData = [ ...bookmarkCampaign.data ];
            const index = campaignData.findIndex(item => item.id == action.payload);
            campaignData.splice(index, 1);
        ...
        case FETCH_PLEDGE_RECEIVED_PENDING: {
            const pledgeReceived = { ...state.pledgeReceived };
        ...
        case FETCH_WITHDRAW_METHODS_PENDING: {
            const withdrawMethod = { ...state.withdrawMethod };
        ...
        case SAVE_WITHDRAW_ACCOUNT_COMPLETE: {
            const withdrawMethod = { ...state.withdrawMethod };
            withdrawMethod.saveReq = 'pending';
            const wMethodData = { ...withdrawMethod.data };
            if( action.payload.success ) {
