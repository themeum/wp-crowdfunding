...
class Order extends Component {
	...
	onClickFilter = (e) => {
		e.preventDefault();
		const filterValue =
			e.target.innerText.toLowerCase() == 'all'
				? ''
	...
	geOrderLength = (key) => {
		...
		if (key) {
			orderLength = data.filter((item) => item.details.status == key)
				.length;
		}
		return orderLength;
	};
	...
	getOrderData = () => {
		...
		if (filterValue) {
			filterData = order.data.filter(
				(item) => item.details.status == filterValue
			);
		}
		if (searchText) {
			filterData = order.data.filter(
				(item) =>
					...
					item.details.billing.first_name
						.toLowerCase()
						.search(searchText.toLowerCase()) !== -1 ||
					item.details.billing.last_name
						.toLowerCase()
						.search(searchText.toLowerCase()) !== -1
			);
		}
				...
				<div className='wpcf-dashboard-content-inner'>
					<div className='wpcf-pledge-received-cards'>
						<div className='wpcf-pledge-received-card'>
							<h4 className='wpcf-dashboard-info-val'>
								{this.geOrderLength()}
							</h4>
						...
						<div className='wpcf-pledge-received-card'>
							<h4 className='wpcf-dashboard-info-val'>
								{this.geOrderLength('pending')}
							</h4>
							<span>Pending Order</span>
						</div>
						<div className='wpcf-pledge-received-card'>
							<h4 className='wpcf-dashboard-info-val'>
								{this.geOrderLength('completed')}
							</h4>
